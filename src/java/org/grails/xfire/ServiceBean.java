...
/**
 * A convenience bean which creates a Service from a ServiceFactory instance. If
 * there is no ServiceFactory set, ServiceBean will create one from the
...
public class ServiceBean implements InitializingBean, ApplicationContextAware {
	...
	public void afterPropertiesSet() throws Exception {
		...
		if (serviceFactory == null) {
			serviceFactory = new ObjectServiceFactory(xFire
					.getTransportManager(), new AegisBindingProvider());
		}
		...
		if (wsdlURL != null) {
			...
			if (url == null) {
				throw new XFireRuntimeException("Could not resolve WSDL URL "
						+ wsdlURL);
			}

			xfireService = serviceFactory.create(intf, null, url, properties);
		} else {
			xfireService = serviceFactory.create(intf, name, namespace,
					properties);
		}
		...
		if (invoker != null) {
			...
			if (serviceBean != null) {
				xfireService.setInvoker(new GroovyBeanInvoker(serviceBean));
			}
		}
	...
	protected void initializeBindings() throws Exception {
		...
		for (Iterator itr = bindings.iterator(); itr.hasNext();) {
			...
			if (o instanceof Soap11BindingBean) {
				binding = osf.createSoap11Binding(xfireService, o.getName(), o
						.getTransport());
			} else if (o instanceof Soap12BindingBean) {
				binding = osf.createSoap12Binding(xfireService, o.getName(), o
						.getTransport());
			}
			...
			for (Iterator eitr = o.getEndpoints().iterator(); eitr.hasNext();) {
				...
				osf.createEndpoint(xfireService, ep.getName(), ep.getUrl(),
						binding);
			}
		}
	}
	...
	/**
	 * Set the service class. The service class is passed to the
	 * ServiceFactory's create method and is used to determine the operations on
	 * the service.
	 * 
	 * @return
	 */
	...
	public void setApplicationContext(ApplicationContext ctx)
		...
		if (serviceFactory == null)
			serviceFactory = (ServiceFactory) ctx
					.getBean("xfire.serviceFactory");
	}
